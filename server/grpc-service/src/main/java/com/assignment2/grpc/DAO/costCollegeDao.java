...
public class costCollegeDao {
    ...
    public void quarryZero() throws IOException {
        ...
        try (CSVReader reader = new CSVReader(new FileReader(path))) {
            ...
            for (String[] row : rows) {
                ...
                for (int j = 0; j < row.length; j = j + 6) {
                    mongodb.document tmp = new mongodb.document(Integer.parseInt(row[j]), row[j + 1], row[j + 2], row[j + 3], row[j + 4], Integer.parseInt(row[j + 5]));
